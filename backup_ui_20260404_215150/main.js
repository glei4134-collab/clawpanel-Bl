...
import { applyUIConfig, getUIConfig, setupClickSounds, applyBgImage, applyBgBlur, applyBgBrightness, applyBgOpacity, applySidebarAlpha, applyChatMainAlpha, applySessionListAlpha } from './lib/ui-custom.js'
...
if (config) {
  if (config.bgBlur) applyBgBlur(config.bgBlur)
  if (config.bgBrightness) applyBgBrightness(config.bgBrightness * 100)
  if (config.bgOpacity) applyBgOpacity(config.bgOpacity * 100)
  if (config.sidebarAlpha) applySidebarAlpha((1 - config.sidebarAlpha) * 100)
  if (config.chatMainAlpha) applyChatMainAlpha(config.chatMainAlpha * 100)
  if (config.sessionListAlpha) applySessionListAlpha(config.sessionListAlpha * 100)
}
...
const _logoSvg = `<div style="width:64px;height:64px;background:linear-gradient(135deg,#667eea,#764ba2);border-radius:16px;display:flex;align-items:center;justify-content:center;font-size:32px;font-weight:bold;color:#fff;box-shadow:0 4px 16px rgba(102,126,234,0.4)">G</div>`
...
async function boot() {
  ...
  ensureWebSession.then(() => loadActiveInstance()).then(() => detectOpenclawStatus()).then(async () => {
    ...
    if (!isOpenclawReady()) {
      ...
      try {
        const panelCfg = await api.readPanelConfig()
        skipAutoConnect = !!panelCfg?.noAutoConnect
      } catch {}

      // 自动连接 WebSocket（如果 Gateway 正在运行且未禁止）
      if (!skipAutoConnect && isGatewayRunning()) {
        autoConnectWebSocket()
      }

      // 监听 Gateway 状态变化，自动连接/断开 WebSocket
      onGatewayChange(async (running) => {
        if (running) {
          let shouldSkip = false
          let shouldSkipReconnect = false
          try {
            const cfg = await api.readPanelConfig()
            shouldSkip = !!cfg?.noAutoConnect
            shouldSkipReconnect = !!cfg?.disableReconnect
          } catch {}
          if (!shouldSkip && !shouldSkipReconnect) autoConnectWebSocket()
      ...
      onInstanceChange(async () => {
        ...
        if (isGatewayRunning() && !shouldSkip && !shouldSkipReconnect) autoConnectWebSocket()
      })
    }
